...
function Search() {
  ...
  const params = url.slice(34)

  useEffect(() => {
    axios.get('http://' + process.env.REACT_APP_URL + ':1117/search?user=' + params)
      .then(res => {
    ...
    <div className='bg-blue-950'>
      <div className="bg-townNN bg-cover h-screen flex flex-col items-center justify-center">
        ...
        <div className="w-1/2  mt-12 bg-transparent">
          <h2 className='text-white font-bold justify-center text-center mt-10 '>{users.length}&nbsp;  R E S U L T S  : </h2>
            ...
            {users.length >0 && users.map((user, index) => (
              <div key={index} className="flex w-full ">
                <div
                  ...
                  <div className='flex-1 mt-3'>  <p>{user.username}</p></div>
                  <div className='flex-1 text-white font-bold mt-3 w-1/6'><p>L V L     {user.lvl}</p></div>
                </div>
